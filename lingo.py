...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if play_again_button_position[0] <= mouse_pos[0] <= play_again_button_position[0] + button_width and play_again_button_position[1] <= mouse_pos[1] <= play_again_button_position[1] + button_height:
    ...
    if not game_over:
        if guess_word[0] == list(target_word):  # Compare only the word, not the colors
            ...
            message = f"You lost! The correct word was: {target_word}"
            game_over = True
            current_win_streak = 0
    ...
    window.blit(attempts_surface, (20, 60))
    ...
    if game_over:
        ...
        window.blit(message_surface, (20, message_y))
        draw_button("Play Again", play_again_button_position)
